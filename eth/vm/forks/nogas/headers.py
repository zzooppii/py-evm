...
from .constants import (
    NOGAS_DIFFICULTY_ADJUSTMENT_CUTOFF
)

if TYPE_CHECKING:
    from eth.vm.forks.nogas import NogasVM    # noqa: F401


def compute_nogas_difficulty(parent_header: BlockHeaderAPI, timestamp: int) -> int:
    """
    ...
    if timestamp - parent_header.timestamp < NOGAS_DIFFICULTY_ADJUSTMENT_CUTOFF:
        base_difficulty = max(
...
def create_nogas_header_from_parent(parent_header: BlockHeaderAPI,
    ...
    if 'difficulty' not in header_params:
        ...
        header_params['difficulty'] = compute_nogas_difficulty(
            parent_header,
...
def configure_nogas_header(vm: "NogasVM", **header_params: Any) -> BlockHeader:
    ...
    with vm.get_header().build_changeset(**header_params) as changeset:
        if 'timestamp' in header_params and vm.get_header().block_number > 0:
            ...
            changeset.difficulty = compute_nogas_difficulty(
                parent_header,
                header_params['timestamp'],
            )
